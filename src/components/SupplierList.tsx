...
export default function SupplierList() {
  ...
  const filteredSuppliers = suppliers.filter(supplier => {
    ...
    const matchesCategory = !filterCategory || supplier.category.toLowerCase().includes(filterCategory.toLowerCase());
  ...
  const handleToggleStatus = (supplier: Supplier) => {
    const updatedSupplier = {
      ...supplier,
      status: supplier.status === 'active' ? 'inactive' : 'active' as 'active' | 'inactive',
          ...
          >
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center">
                <div className={`p-2 rounded-lg ${supplier.cnpj ? 'bg-blue-50' : 'bg-green-50'}`}>
                  {supplier.cnpj ? (
                    <Building className={`w-5 h-5 ${supplier.cnpj ? 'text-blue-600' : 'text-green-600'}`} />
                  ) : (
                    <User className={`w-5 h-5 ${supplier.cnpj ? 'text-blue-600' : 'text-green-600'}`} />
                  )}
                </div>
                <div className="ml-3">
              ...
              >
                {supplier.status === 'active' ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
              </button>
            </div>
            ...
            {(supplier.cnpj || supplier.cpf) && (
              <div className="mb-3">
                <p className="text-sm text-gray-600">
                  {supplier.cnpj ? 'CNPJ' : 'CPF'}: {formatDocument(supplier.cnpj, supplier.cpf)}
                </p>
              </div>
            )}
