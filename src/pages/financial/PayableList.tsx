...
export default function PayableList() {
  usePageTitle('Contas a Pagar | ContaCerta');
  const { state, dispatch } = useApp();
  const confirm = useConfirm();
  const { documents = [], suppliers = [], costCenters = [] } = state;
  
  const [searchTerm, setSearchTerm] = useState('');
  const [filterStatus, setFilterStatus] = useState<'all' | 'open' | 'paid' | 'overdue'>('all');

  const payableDocuments = documents.filter(doc => doc.type === 'payable');
  ...
  const filteredDocuments = payableDocuments.filter(doc => {
    const supplierName = getSupplierNameById(doc.supplierId).toLowerCase();
  ...
  const getStatusInfo = (status: string) => {
    switch (status) {
      case 'paid':
        return { icon: CheckCircle, text: 'Pago', color: 'text-green-700 bg-green-50', iconColor: 'text-green-500' };
      case 'overdue':
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        {filteredDocuments.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-gray-500">Tente ajustar os filtros ou criar uma nova conta a pagar.</p>
          </div>
        )}
      </div>
    </div>
